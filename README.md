...
**只暂存本次会话中 `edit`/`write` 实际写过的文件，不用 `git add -A`；且按「文件各自所在的仓库」分别提交。**
...
**v2 修正（2026-09-16）**：旧实现用**会话 cwd** 解析仓库（`git rev-parse`），于是
① cwd 不是仓库时直接 return ⇒ **从不提交**（实测：从非仓库目录跑 pi 去改
`~/.pi/agent` 里的配置，仓库历史里 `[pi] 改动` 提交数为 **0**）；② 只有 cwd 仓库内的
文件会被提交，**跳仓库的改动被静默丢弃**。现在对每个被触及文件各自向上找 toplevel，
按仓库分组提交；不在任何仓库里的文件跳过。

两条额外防线：`git add` 被拒（路径被 .gitignore 忽略）→ 跳过该仓库并提示，**不用 `-f` 强加**；
该仓库正在 merge/rebase（存在 `MERGE_HEAD`/`REBASE_HEAD`/`CHERRY_PICK_HEAD`）→ 跳过，
避免插入提交打乱序列。
实测（真实 git，cwd 为非仓库）：两个仓库各自拿到 `[pi] 改动 N 个文件：…` 提交、
非仓库目录不提交、重复 shutdown 不产生空提交、忽略路径与 merge 场景均正确跳过（12/12）。
